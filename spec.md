...
24. As a Staff member, I want to manually reserve a Schedule Slot for walk-in or telephone bookings, so that off-platform reservations are accurately reflected in the system.
25. As a Staff member, I want to apply a Slot Block to any Schedule Slot, so that courts undergoing routine maintenance or hosting internal club tournaments cannot be reserved by the public.
26. As a Staff member, I want to remove an active Slot Block when maintenance concludes early, so that the Court becomes immediately available for booking.
27. As a Staff member, I want the system to automatically transition confirmed Bookings to No-Show status if Check-in has not occurred by the end of the Schedule Slot, so that attendance reports remain accurate.
28. As a Staff member, I want to see a Flagged indicator next to Customer records that have accumulated 3 or more No-Show events, so that I can address attendance reliability.
29. As a Staff member, I want to view the real-time allocation of the Equipment Pool across all slots, so that on-site staff can stage the correct number of rackets and balls at the front desk.
30. As a Staff member, I want to receive an urgent dashboard notification when a late payment conflict is flagged, so that I can manually contact the Customer and assign an alternative slot.
...
Standard integration test conventions for transactional web services:
- API route test suites driving HTTP requests and asserting on JSON responses and database state transitions.
- Redis-backed distributed lock test harnesses verifying mutual exclusion under parallel `Promise.all` executions.
